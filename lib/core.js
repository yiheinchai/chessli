const CHESS_COM_GAME_URL = /^https:\/\/(?:www\.)?chess\.com\/game\/(?:live\/)?(\d+)(?:[/?#]|$)/i;
const LICHESS_IMPORT_TOKEN = /^[A-Za-z0-9_-]{16,80}$/;
...
export function lichessGameUrl(url) {
  if (typeof url !== "string") return null;
  try {
    const parsed = new URL(url);
    if (parsed.origin !== "https://lichess.org") return null;
    const gameId = parsed.pathname.split("/").filter(Boolean)[0];
    if (gameId === "analysis" || !/^[A-Za-z0-9]{8}$/.test(gameId ?? "")) return null;
...
export function lichessPasteUrl(token) {
  if (!LICHESS_IMPORT_TOKEN.test(token ?? "")) {
    throw new Error("The Lichess import token is invalid.");
  }
  return `https://lichess.org/paste#chessli=${encodeURIComponent(token)}`;
}
